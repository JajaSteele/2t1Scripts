...
local main_menu = menu.add_feature("JJS Translator", "parent", 0)
...
local send_menu = menu.add_feature("Send Message", "parent",main_menu.id)

local msg_lang = menu.add_feature("Message Language: [en]", "action", send_menu.id, function(feat)
    local status = 1
    while status == 1 do
...
local msg_org_toggle = menu.add_feature("Org/MC/Team only", "toggle", send_menu.id)
