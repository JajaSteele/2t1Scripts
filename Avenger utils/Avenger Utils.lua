...
local register_avenger = menu.add_feature("Find and Register Avenger","action",main_menu.id,function()
    avenger = find_avenger()
    if avenger ~= 0 then
...
register_avenger.hint = "Will only work if your Avenger's blip is visible (So must be outside of the avenger)"
    ...
    if not autopilot_active then
        if avenger ~= 0 then
            ...
            if travel_dist > 2500 then
                ...
                native.call(0xF7F9DCCA89E7505B, avenger_pilot, avenger, avenger_pos+v3(0,0,50), math.ceil(avenger_pos.z+175), 160, true, forward_heading, true)
...
local tp_avenger = menu.add_feature("TP on Avenger","action",tp_avenger_menu.id, function()
    ...
    if avenger ~= 0 then
        ...
        menu.notify("No avenger found!","Error",nil,0x0000FF)
    end
end)
